---
title: "Variable selection for treatment classification"
...
- Fit simple models, test on opposite years - see test_important_variables.Rmd
...
extract_vi_from_random_forests <- function(in_data = mod_data_2020) {
  ...
  for(i in 1:length(random_seeds)){
    ...
    rf_recipe <- recipe(treatment ~ . , data = in_data) |> 
...
```{r}
vip |> 
  write_csv("out/metrics/variable_importance_classify_di.csv")
```
